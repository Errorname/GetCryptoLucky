	...
	$.ajax($('#slots_html').attr('src'),{
		success: function(data) {
			...
			for(var i in this.cols) {
				...
				var firstThree = '';
				for(var x = 0; x < 30; x++) {
					var part = '<svg class="coin"><use xlink:href="#icon-'+CryptoSlots.icons[Math.floor(Math.random()*CryptoSlots.icons.length)]+'"></use></svg>';
					str += part
...
CryptoSlots.prototype.spin = function(draw) {
	if (this.options.before_spin)
		this.options.before_spin();
...
CryptoSlots.prototype.stop = function() {
	...
	setTimeout(function() {
		...
		setTimeout(function() {
			if (this.options.after_spin)
				this.options.after_spin();
		}.bind(this),2600);
	}.bind(this),50);
}
